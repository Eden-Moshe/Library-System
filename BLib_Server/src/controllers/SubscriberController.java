...
public class SubscriberController {
    ...
    /**
     * Edits a subscriber's information in the database.
     *
     * @param pKey The subscriber's unique identifier.
    ...
    public void editSubscriber(String pKey, String field, String val) {
        ...
        if (!validCol.contains(field)) {
            System.out.println("editSubscriber: Invalid field");
            return;
        }
        ...
        try {
            if (testCreation.next()) {
                if (!testCreation.getString("user_password").equals(tempPassword)) {
                    System.out.println("Failed to create new subscriber. Likely a collision occurred. Trying again.");
                    return addSubscriber(newSub);  // Retry adding the subscriber.
                } else {
                    String[] subFields = {
                            "subscriber_id", "subscriber_name", "subscriber_phone_number", "subscriber_email", "subscriber_status"
                    };
                    String[] subValues = {
                            newSub.getSID(), newSub.getName(), newSub.getPNumber(), newSub.getEmail(), newSub.getStatus()
                    };
